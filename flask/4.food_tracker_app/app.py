...
def index():
    ...
    cur = db.execute('select log_date.entry_date, sum(food.protein) as protein, sum(food.carbohydrates) as carbohydrates, '
                     'sum(food.fat) as fat, sum(food.calories) as calories from log_date join food_date on food_date.log_date_id = log_date.id'
                     ' join food on food.id = food_date.food_id where log_date.entry_date = entry_date group by log_date.id order by log_date.entry_date desc')
    ...
    for result in results:
        ...
        single_date['carbohydrates'] = result['carbohydrates']
...
def food():
    ...
    if request.method == 'POST':
        ...
        db.commit()
        # return '<h1>Name: {} Protein: {} Carbs: {} Fat: {}'.format(request.form['food-name'], request.form['protein'],
